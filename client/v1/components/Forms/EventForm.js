...
class EventForm extends Component {
  ...
  render() {
    const {
      errors, categoryIsValid, timezoneIsValid, timezone,
    } = this.state;
    const {
      formId, formData, categories, slackChannels,
    } = this.props;
    const listChannels = slackChannels && slackChannels.map(channel => this.renameKey('name', 'title', channel));
    const {
      formData: {
        title, description, venue, featuredImage, category, slackChannel,
      },
    } = this.state;
...
const mapDispatchToProps = dispatch => bindActionCreators({ getSlackChannelsList }, dispatch);

const mapStateToProps = state => ({ slackChannels: state.slackChannels.channels });
